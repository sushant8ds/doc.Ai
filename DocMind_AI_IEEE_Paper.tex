...
% ---- Authors ------------------------------------------------
% Row 1: Faculty Guide (top, full width)
% Row 2: Three students side-by-side
\author{
  % ── Faculty Guide (first / top row) ────────────────────────
  ...
  \and
  % ── Student 1 ──────────────────────────────────────────────
  \IEEEauthorblockN{Sushant Tiwari}
  \IEEEauthorblockA{
    \textit{Dept.\ of CSE}\\
    \textit{KLE Technological University}\\
    Hubballi, Karnataka, India\\
    sushantds2003@gmail.com}
  \and
  % ── Student 2 ──────────────────────────────────────────────
...
Transformer-based generative AI pipelines have demonstrated strong
capability in clinical knowledge extraction from electronic health
records~\cite{naik2025healing}, illustrating the breadth of tasks
addressable by locally-deployed language models.

\subsection{Generative AI for Structured Data}
Recent work has applied generative models beyond text question answering.
Patil et al.~\cite{patil2025gan} demonstrate GAN-based sentiment
analysis of product reviews, showing that adversarial training improves
classification robustness.
Kore et al.~\cite{kore2026cgan} employ conditional GANs for personalised
image and video synthesis, while Reddy et al.~\cite{reddy2026dcgan}
combine DCGANs with feature-based explainability for synthetic face
generation and fake-image detection.
These works collectively underscore the growing role of generative AI
across diverse modalities — from text and structured data to images —
motivating the design of privacy-preserving local pipelines such as
DocMind AI.
...
\begin{tikzpicture}[
  ...
  box/.style  = {rectangle, draw=black!55, rounded corners=4pt,
                 minimum height=0.7cm, minimum width=3.0cm,
                 text centered, font=\small, fill=white},
  grp/.style  = {draw=black!35, dashed, rounded corners=6pt, inner sep=5pt},
  arr/.style  = {-{Stealth[length=5pt]}, thick},
  darr/.style = {-{Stealth[length=4pt]}, thick, dashed, gray!70},
]

%% ── TOP: User ─────────────────────────────────────────────────────
\node[box, fill=gray!18, minimum width=4.2cm] (user)
      {\textbf{User} \quad PDF upload / Chat queries};

%% ── ROW 1: Frontend components ───────────────────────────────────
\node[box, fill=blue!12, minimum width=3.8cm,
      below=0.6cm of user, xshift=-5.0cm] (chat)
      {\textbf{Chat Pane}\\
       {\small streaming answer · citation badges}\\
       {\small key points · hallucination badge}};

\node[box, fill=blue!8, minimum width=2.8cm,
      below=0.6cm of user] (zustand)
      {\textbf{Zustand Store}\\
       {\small IDLE / UPLOAD}\\
       {\small INDEX / CHAT}};

\node[box, fill=blue!12, minimum width=3.8cm,
      below=0.6cm of user, xshift=5.0cm] (pdfpane)
      {\textbf{PDF Viewer Pane}\\
       {\small react-pdf · canvas overlay}\\
       {\small page nav · source highlights}};

\begin{scope}[on background layer]
  \node[grp, fill=blue!4,
        fit=(chat)(zustand)(pdfpane),
        label={[font=\small\bfseries]above:Presentation Layer ---
               React~18 + TypeScript + Vite (port~5173)}] (ui) {};
\end{scope}

%% ── ROW 2: REST API bar ───────────────────────────────────────────
\node[box, fill=green!14, minimum width=15cm, minimum height=0.9cm,
      below=0.55cm of ui.south] (api)
      {\textbf{REST API Layer} \quad FastAPI + Uvicorn \quad port 8000\\
       {\small \texttt{GET /health} \quad \texttt{POST /upload}
               \quad \texttt{POST /chat} \quad \texttt{POST /reset}}};

%% ── ROW 3 LEFT: Document Indexing Pipeline ───────────────────────
\node[box, fill=orange!18, minimum width=3.8cm,
      below=0.55cm of api.south, xshift=-5.5cm] (s1)
      {\textbf{Stage 1 — Extraction}\\PyMuPDF page-aware chars};
\node[box, fill=orange!14, minimum width=3.8cm, below=0.38cm of s1] (s2)
      {\textbf{Stage 2 — Chunking}\\$C{=}500$, $O{=}50$, stride $S{=}450$};
\node[box, fill=orange!14, minimum width=3.8cm, below=0.38cm of s2] (s3)
      {\textbf{Stage 3 — Embedding}\\MiniLM $E\!\in\!\mathbb{R}^{N\times384}$};
\node[box, fill=orange!18, minimum width=3.8cm, below=0.38cm of s3] (s4)
      {\textbf{Stage 4 — Indexing}\\FAISS \texttt{IndexFlatL2}};

\begin{scope}[on background layer]
  \node[grp, fill=orange!5,
        fit=(s1)(s2)(s3)(s4),
        label={[font=\small\bfseries]above:Document Indexing Pipeline}]
        (docpipe) {};
\end{scope}

%% ── ROW 3 RIGHT: RAG Query Pipeline ──────────────────────────────
\node[box, fill=red!10, minimum width=3.8cm,
      below=0.55cm of api.south, xshift=5.5cm] (r1)
      {\textbf{Step 1 — Query Encode}\\$\mathbf{q}=\phi(q)\!\in\!\mathbb{R}^{384}$};
\node[box, fill=red!10, minimum width=3.8cm, below=0.38cm of r1] (r2)
      {\textbf{Step 2 — FAISS Search}\\top-$k{=}5$ chunks + pages};
\node[box, fill=red!10, minimum width=3.8cm, below=0.38cm of r2] (r3)
      {\textbf{Step 3 — Prompt Build}\\\texttt{[1]\ldots[5]} context template};
\node[box, fill=red!22, minimum width=3.8cm, below=0.38cm of r3] (r4)
      {\textbf{Step 4 — LLM Generate}\\Ollama LLaMA~3.2 (3B Q4)};
\node[box, fill=green!16, minimum width=3.8cm, below=0.38cm of r4] (r5)
      {\textbf{Step 5 — Insights}\\key points + follow-up Qs};
\node[box, fill=yellow!35, minimum width=3.8cm, below=0.38cm of r5] (r6)
      {\textbf{Step 6 — Hallucination}\\verdict + confidence score};

\begin{scope}[on background layer]
  \node[grp, fill=red!4,
        fit=(r1)(r2)(r3)(r4)(r5)(r6),
        label={[font=\small\bfseries]above:RAG Query Pipeline}]
        (ragpipe) {};
\end{scope}

%% ── ROW 3 CENTRE: In-memory Session ─────────────────────────────
\node[box, fill=gray!12, minimum width=2.8cm,
      below=0.55cm of api.south] (session)
      {\textbf{Session State}\\
       \texttt{index}\\
       \texttt{chunks[]}\\
       \texttt{chunk\_pages[]}\\
...
%% ── Arrows ────────────────────────────────────────────────────────
\draw[arr] (user.south) -- ++(0,-0.15) -| (chat.north);
\draw[arr] (user.south) -- ++(0,-0.15) -| (pdfpane.north);
\draw[darr](chat.east)  -- (zustand.west);
\draw[darr](zustand.east) -- (pdfpane.west);
\draw[arr] (ui.south)   -- node[right,font=\scriptsize]{HTTP/REST} (api.north);
\draw[arr] (api.south)  -| (docpipe.north);
\draw[arr] (api.south)  -- (session.north);
\draw[arr] (api.south)  -| (ragpipe.north);
\draw[arr] (s1)--(s2); \draw[arr](s2)--(s3); \draw[arr](s3)--(s4);
\draw[arr] (r1)--(r2); \draw[arr](r2)--(r3); \draw[arr](r3)--(r4);
\draw[arr] (r4)--(r5); \draw[arr](r5)--(r6);
% FAISS index shared with search
\draw[darr](s4.east) -- ++(0.35,0) |- node[right,font=\tiny]{shared index}(r2.east);
% session shared with RAG
\draw[darr](session.east) -- ++(0.2,0) |- (r2.west);

\end{tikzpicture}
}% end resizebox
\caption{Complete DocMind AI system architecture.
         Left: four-stage document indexing pipeline (offline, runs once per PDF).
         Centre: FastAPI REST layer and in-memory session state.
         Right: six-step RAG query pipeline (online, runs per query).
         Top: React/TypeScript dual-pane frontend.
         Dashed arrows denote shared state; solid arrows denote data flow.}
\label{fig:arch}
\end{figure*}
...
\bibitem{naik2025healing}
G.~Naik, S.~Bendigerimath, V.~Kawari, G.~Narajji, and G.~Konnurmath,
``Healing through language: Transformer-based Gen AI pipeline with T5
for clinical knowledge extraction from EHRs,''
in \emph{Proc.\ 2025 IEEE 4th Int.\ Conf.\ for Advancement in
Technology (ICONAT)}, pp.~1--6, 2025,
doi: 10.1109/ICONAT66879.2025.11362713.

\bibitem{patil2025gan}
V.~Patil, V.~S.~Hiremani, A.~Mulimani, S.~R.~Kolagal, and
G.~Konnurmath,
``GAN based sentiment analysis of Amazon reviews,''
in \emph{Proc.\ 2025 IEEE 4th Int.\ Conf.\ for Advancement in
Technology (ICONAT)}, pp.~1--7, 2025,
doi: 10.1109/ICONAT66879.2025.11362707.

\bibitem{kore2026cgan}
S.~Kore, S.~Nayak, P.~Kalakamb, S.~Doddamani, and G.~Konnurmath,
``Personalised image and video synthesis using conditional generative
adversarial networks,''
in A.~Chakravorty, S.~Hussain, R.~Kumari (Eds.),
\emph{Artificial Intelligence: Theory and Applications (AITA 2025)},
Lecture Notes in Networks and Systems, vol.~1865,
Springer, Cham, 2026,
doi: 10.1007/978-3-032-19185-4\_7.

\bibitem{reddy2026dcgan}
K.~K.~K.~Reddy \emph{et al.},
``Synthetic human face generation using DCGAN and fake image
classification with feature-based explanation,''
in A.~Chakravorty, S.~Hussain, R.~Kumari (Eds.),
\emph{Artificial Intelligence: Theory and Applications (AITA 2025)},
Lecture Notes in Networks and Systems, vol.~1865,
Springer, Cham, 2026,
doi: 10.1007/978-3-032-19185-4\_1.
