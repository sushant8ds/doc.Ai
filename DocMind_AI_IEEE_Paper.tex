...
\begin{tikzpicture}[node distance=0.45cm and 0.3cm]
  ...
  % --- Presentation Layer ---
  \node[block, fill=blue!10, minimum width=3.8cm] (chat)
        {Chat Pane\\{\scriptsize streaming · citations · hallucination}};
  \node[block, fill=blue!10, minimum width=3.8cm, right=0.25cm of chat] (pdf)
        {PDF Viewer Pane\\{\scriptsize react-pdf · canvas highlights}};
  \node[draw, dashed, rounded corners, fit=(chat)(pdf),
        label=above:{\small\textbf{Presentation Layer (React/TS)}}] (ui) {};

  % --- API Layer ---
  \node[block, fill=green!10, minimum width=8.1cm,
        below=0.55cm of ui.south] (api)
        {\textbf{REST API Layer} — FastAPI + Uvicorn :8000\\
         {\scriptsize POST /upload \quad POST /chat \quad GET /health \quad POST /reset}};

  % --- Backend boxes ---
  \node[block, fill=orange!10, minimum width=3.5cm,
        below left=0.55cm and -1.6cm of api.south] (docpipe)
        {\textbf{Document Pipeline}\\
         {\scriptsize 1.\ PyMuPDF Extract}\\
         {\scriptsize 2.\ Page-Aware Chunk}\\
         {\scriptsize 3.\ MiniLM Embed}\\
         {\scriptsize 4.\ FAISS Index Build}};

  \node[block, fill=red!8, minimum width=3.5cm,
        below right=0.55cm and -1.6cm of api.south] (ragpipe)
        {\textbf{RAG Pipeline}\\
         {\scriptsize Encode Query}\\
         {\scriptsize FAISS Search (top-5)}\\
         {\scriptsize Prompt Builder}\\
         {\scriptsize Ollama LLaMA 3.2}\\
         {\scriptsize Hallucination Check}};
...
\end{tikzpicture}
\caption{DocMind AI three-tier system architecture.}
\label{fig:arch}
\end{figure}
...
\begin{equation}
  ...
  \label{eq:pagelabel}
\end{equation}
where $\mathrm{page}(\cdot)$ maps a character position to its
originating PDF page number via the pre-computed \texttt{char\_page}
array.
...
\begin{equation}
  ...
  \label{eq:prompt}
\end{equation}
where $\|$ denotes string concatenation and $\bigoplus$ denotes
sequential concatenation over retrieved chunks.
...
\begin{equation}
  ...
  \label{eq:gen}
\end{equation}
with nucleus sampling selecting from the smallest vocabulary subset
$\mathcal{V}' \subseteq \mathcal{V}$ satisfying:
\begin{equation}
  \sum_{v \in \mathcal{V}'} P_\theta(v \mid \mathrm{context}) \geq p = 0.9
  \label{eq:nucleus}
\end{equation}
Generation parameters: \texttt{num\_ctx}$=2048$,
\texttt{top\_k}$=40$, \texttt{top\_p}$=0.9$.

\subsection{Hallucination Verification}
The hallucination module computes a grounding verdict
$v \in \{\texttt{SUPPORTED},\allowbreak\texttt{PARTIALLY\_SUPPORTED},\allowbreak\texttt{NOT\_SUPPORTED}\}$
...
\begin{equation}
  ...
  \label{eq:halluc}
\end{equation}
The grounding ratio is approximated as:
\begin{equation}
  ...
  \label{eq:gr}
\end{equation}
where $s \sqsubseteq c$ denotes semantic entailment of claim $s$ by
chunk $c$.
